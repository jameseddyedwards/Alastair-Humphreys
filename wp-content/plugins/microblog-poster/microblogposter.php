        ...
        if(!empty($pinterest_accounts))
        {
            ...
            foreach($pinterest_accounts as $pinterest_account)
            {
                ...
                if(isset($extra['access_token']) && $featured_image_src_full)
                {
                    ...
                    $access_token = "Bearer " . $access_token_short;
                    $body = new stdClass();
                    $body->board = $pinterest_account['username'].'/'.$extra['board_name'];
                    $body->note = $update;
                    $body->link = $permalink;
    ...
    {   
        ...
        if(!empty($flickr_accounts))
        {
            foreach($flickr_accounts as $flickr_account)
            {
                ...
                {
                    $flickr_account['message_format'] = str_ireplace('{manual_excerpt}', '', $flickr_account['message_format']);
                    $flickr_account['message_format'] = str_ireplace('{excerpt}', '', $flickr_account['message_format']);
